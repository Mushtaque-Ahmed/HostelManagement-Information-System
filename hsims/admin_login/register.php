...
if (isset($_POST['submit'])) {
	...
	if ($password == $cpassword) {
		...
		if (!$result->num_rows > 0) {
			$sql = "INSERT INTO admins (username, email, password, contact, id)
					VALUES ('$username', '$email', '$password', '$contact', '$id')";
			$result = mysqli_query($conn, $sql);
			if ($result) {
	...
	<div class="container">
		<form action="" method="POST" class="login-email">
			...
			<div class="input-group">
				<input type="number" placeholder="Admin Id" name="id" value="<?php echo $id; ?>" required>
			</div>
			<div class="input-group">
				<input type="text" placeholder="Username/Full name(In block letters)" name="username" value="<?php echo $username; ?>" required>
			</div>
			<div class="input-group">
